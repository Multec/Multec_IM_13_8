...
public class EventModel {
	...
	public ArrayList<ListItem> getEvents() {
		...
		try {
			dbQuery = new DatabaseQuery("SELECT * FROM events ORDER BY Event_date, Event_name;");
		...
		try {
			while(queryResult.next()) {
				events.add(new ListItem(queryResult.getString("Event_thumb_path"), queryResult.getString("Event_name"), "", "", df.format(queryResult.getDate("Event_date"))));
			}
	...
	public ArrayList<Cluster> getClusters() {
		...
		DatabaseQuery dbQuery = null;
		try {
			dbQuery = new DatabaseQuery("SELECT count(*), Event_date FROM events GROUP BY (Event_date >= now());");
			queryResult = dbQuery.getResult();
		} catch (SQLException e) {
			System.out.println("There was an error while executing the query. Error:" + e);
		}
		...
		try {
			while(queryResult.next()) {
				if (queryResult.getDate("Event_date").before(new Date())) {
					clusters.add(new Cluster("Oude events", queryResult.getInt("count(*)")));
				} else {
					clusters.add(new Cluster("Aankomende events", queryResult.getInt("count(*)")));
				}
			}
